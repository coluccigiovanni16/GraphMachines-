...
def train_classification(net, data_set_train, data_set_test, optimizer, num_epochs, d_value, criterion):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (int(num_epochs / 1)) == 0:  # print every (num_epochs/10) epochs --> total 10 print
            ...
            loss_test.append(predict_regression(net, data_set_test, d_value, criterion, rmse=True))
    ...
    print('FINE TRAINING')
    print('\n+++++++++++++++++++++++++++++++++++++++\n\n')
    print('LOSSTest', loss_test)
    print('LOSSTrain', loss_train)
    return loss_train, loss_test, training_time
