...
def fragment_projection(graph: Graph, size: int = 1, candidate_rule: str = "$degree",
                        branch_rule: str = "$depth > tree.parent_modality > $lexic") -> List[CGraph]:
    """
        fragment_projection
        ====================
    """
...
def map_cgraph(graph: Graph, size: int = 1, candidate_rule: str = "$degree",
               branch_rule: str = "$depth > tree.parent_modality > $lexic") -> List[Tuple[str, CGraph]]:
    """
        fragment_projection
        ====================
    """
...
def extract_ngrams(graph: Graph, id_root: str, window_size: int = 1, fragment_size: int = 1,
                   candidate_rule="$degree", branch_rule="$depth > tree.parent_modality > $lexic") -> List[
    List[CGraph]]:
    """
        extract_ngram
        ==============
