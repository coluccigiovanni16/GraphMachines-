...
class Graph:
    """
        ...
        A dictionary-like structure is preferred over a Matrix representation,
    ...
    def __eval__(self, arg: str, id_node: int) -> float:
        """
            ...
            into actual scalar or literal values.
    ...
    def to_dag(self, id_root: str, branch_rule: str = "$root.label > $depth > $lexic", ids_ignore: List = [],
               compact_form=False, allow_hashes=True) -> "Graph":
        """
            to_dag
            -------
    ...
    def group_by_floor(self, id_root: int):
        ...
        def floorgroup_propagation(node_from: Node, node_to: Node, msg: str) -> bool:
            ...
            if not "floor" in node_to.meta or node_to.meta["floor"] is None:
                ...
                return self.broadcast(id_node_from=node_to.id, id_origin=node_from.id, msg=floor + 1,
                                      callback=floorgroup_propagation)
    ...
    def score_cobounds(self, cobounds, allow_hashes: bool = True) -> List:
        ...
        for id_edge in cobounds:
            trace("scoring cobound : %s [%s-%s] - (%s/%s)" % (
                id_edge, self.E[id_edge].id_a, self.E[id_edge].id_b, str(done), str(len(cobounds))), 3)
    ...
    def find_inbounds(self) -> List:
        ...
        for id_node in self.V.keys():
            ...
            upstairs = [route["edge"].id
                        for route in routes
                        if int(self.V[route["to"]].meta.get('floor')) < int(
                    self.V[id_node].meta.get('floor'))]  # ascending only (avoid duplicate)
            if (len(upstairs) > 1):
    ...
    def fix_inbound(self, inbound, id_mirror: str, scoring: "TreeScoring" = "$lexic", mode: str = "duplicate") -> bool:
        ...
        if mode == "duplicate":
            ...
            for id_descendant in ids_children:
                trace("lowering floor of %s to %s" % (
                    id_descendant, str(subdag.V[id_descendant].meta["floor"] + floor_sub)), 4)
    ...
    def get_magnet(self, id_node: str, ignore_virtuals: bool = False, allow_hashes: bool = True) -> str:
        ...
        """
        trace("getting magnet for node #%s, ignore_virtuals = %s, allow_hashes = %s" % (
            id_node, ignore_virtuals, allow_hashes), 3)
        # ensure graph is floored
        if not 'floored_by' in self.meta or self.meta['floored_by'] is None:
    ...
    def to_tree(self, id_root: str, id_origin: str = None, modality_origin: str = None,
                ids_ignore: List[str] = []) -> Tree:
        """
            to_tree
            --------

            Translates an acyclic subgraph into an "agnostic" (unordered) 7 tree form
            A neuwick form is :
                - node centered (root)
        ...
        trace("building a tree rooted on %s (lvl %s), initiated by %s (lvl %s)" % (
            id_root, str(self.V[id_root].meta.get('floor')), id_origin,
            str(self.V[id_origin].meta.get('floor')) if id_origin is not None else "none"), 3)
        if self.is_leaf(id_root) and id_origin is not None:
            ...
            children = [(self.to_tree(id_root=link["to"], id_origin=id_root, modality_origin=link["edge"].modality,
                                      ids_ignore=ids_ignore), link["edge"].modality)
