...
class Tree:
    ...
    def __repr__(self):
        if "trace" in self.meta:
            ...
            if self.is_leaf():
                ...
                return "(" + ', '.join([str(branch) + ":" + str(modality) for (branch, modality) in
                                        sorted(self.children,
                                               key=lambda child: (child[1], child[0].root.label))]) + ")" + lbl
            else:
                # print("(" + ', '.join( [ str(branch) + ":" + str(modality) for (branch, modality) in sorted(self.children, key = lambda child: child[0].meta['score']) ] ) + ")" + lbl)
                trace = "(" + ', '.join([str(branch) + ":" + str(modality) for (branch, modality) in
                                         sorted(self.children, key=lambda child: child[0].meta['score'])]) + ")" + lbl
    ...
    def get_order_sequence(self, prop=None):
        ...
        if self.is_leaf():
            ...
            self.score_tree()
            seq = [lbl, [branch.get_order_sequence(prop) for (branch, modality) in
                         sorted(self.children, key=lambda child: child[0].meta['score'])]]
        else:
            seq = [lbl, [branch.get_order_sequence(prop) for (branch, modality) in
                         sorted(self.children, key=lambda child: child[0].meta['score'])]]
        return seq
                ...
                for id_node in submap:
                    ...
                    if id_node_sanitized:
                        if id_node_sanitized in mapping:
                            ...
                            mapping[id_node_sanitized] = sorted(
                                flatten([mapping[id_node_sanitized]] + [submap[id_node]]))
